...
export function ProjectCard({ project, index, className }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-3xl bg-zinc-900/50 border border-white/10 shadow-2xl shadow-primary/20",
        className,
      )}
    ...
    >
      {/* Spotlight Overlay */}
      <motion.div
        className="pointer-events-none absolute -inset-px rounded-3xl opacity-30 md:opacity-0 transition duration-300 group-hover:opacity-100 z-30"
        style={{
          ...
          `,
          opacity: touchActive ? 1 : undefined,
        }}
      />
      ...
      <div className="relative z-20 mt-auto p-6 flex flex-col h-full justify-end transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
        ...
        <div className="absolute top-6 right-6 opacity-90 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-500 translate-x-0 md:translate-x-4 md:group-hover:translate-x-0">
          <a
            href={project.link}
            target="_blank"
        ...
        <div className="space-y-3">
          ...
          <div className="md:group-hover:opacity-0 transition-opacity duration-300 absolute bottom-6 left-6">
      ...
      {hasMultipleImages && (
        <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-4 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 z-50 pointer-events-none">
          <button
            onClick={prevImage}
